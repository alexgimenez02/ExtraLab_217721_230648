...
float sdfScene(vec3 position) {
    ...
    vec3 rhombus_pos = position + vec3(0.0,0.1*snoise(vec4(position*10,u_time)),0.0);
    ...
    float dist_cylinder = sdfSphere(position + vec3(0.0,-1.0,0.0), vec3(0.0,cos(u_time*3)*2.5 - 0.2,0.0) ,0.2);
    dist = opSmoothUnion(dist_u, dist_rhombus, 2.0);
    dist = opSubtraction(dist_capsule,dist);
    dist = opUnion(dist, dist_cylinder);
    return dist;
}
